...
import { GetProduct } from "../types/product";
  ...
  export const AllProduct = ({data, isLoading}: ProductProps) => {
    ...
    <Card key={index} placeholder={""} className="mt-6 w-96 animate-pulse">
      <CardHeader
        placeholder={""}
        shadow={false}
        floated={false}
    ...
    return (
      ...
      {dummyData.map((product) => (
        <Card key={product.id} placeholder={""} className="w-full max-w-[26rem] shadow-lg mb-4">
          ...
          <CardBody placeholder={""}>
            <div className="mb-3 flex items-center justify-between">
              <Typography placeholder={""} variant="h5" color="blue-gray" className="font-medium font-poppins">
                {product.title}
              </Typography>
            </div>
            ...
            <div className="flex gap-2 mt-2">
              {product.subTypes.map((subType, index) => (
                <Chip key={index} variant="outlined" value={subType} className="font-raleway" />
              ))}
            </div>
          </CardBody>
